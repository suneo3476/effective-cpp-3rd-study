...
1. **非仮想デストラクタとは？** → `virtual` がついていないデストラクタ。ポインタの型のデストラクタしか呼ばれない。

2. **非仮想デストラクタで delete すると？** → エラーは出ない。派生クラスのデストラクタが呼ばれず、メモリリーク。

3. **メモリリークとは？** → 借用したメモリを返却し忘れること。図書館の本の返し忘れ。

4. **ヒープ領域とは？** → `new` で確保する自由領域。ホームセンターの広い駐車場。

5. **スタックとは？** → ローカル変数が置かれる領域。家の細長い駐車場（奥から順番に）。

6. **テキスト領域には何が載る？** → 機械語（バイナリ）。ソースコードではない。

7. **vptr は何に使う？** → 仮想関数を呼ぶとき、実行時にどの関数を呼ぶか決めるため。

8. **ポインタがなぜ 64bit？** → 64bit アーキテクチャで 4GB 以上のメモリを指すため。
